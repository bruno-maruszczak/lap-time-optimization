...
# resolution of "to be saved" plots
dpi = 500
...
def plot_path(dest, left, right, samples, control=None, show_cones=False):
    ...
    """
    plt.figure()
    plt.plot(left[0], left[1], color=LCLR,
             linestyle='solid', zorder=1, linewidth=1)
    plt.plot(right[0], right[1], color=LCLR,
             linestyle='solid', zorder=1, linewidth=1)
    plt.plot(samples[0], samples[1], color='tab:green',
...
def plot_trajectory(dest, left, right, samples, velocities):
    ...
    plt.figure()
    plt.plot(left[0], left[1], color=LCLR,
             linestyle='solid', linewidth=1, zorder=1)
    plt.plot(right[0], right[1], color=LCLR,
             linestyle='solid', linewidth=1, zorder=1)
    p = samples.T.reshape(-1, 1, 2)
    segments = np.concatenate([p[:-1], p[1:]], axis=1)
