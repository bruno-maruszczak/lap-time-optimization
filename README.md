...
First, a high level controller, that takes in a list of points that describe a race track, and according to vehicle constrains generates an optimal trajectory that minimises lap time. This part is based on code by Joe Davison, which we expanded on by adding two additional methods of optimization: Bayesian and non-linear (COBYLA).
...
#### TODO, decsribe the other simple methods
